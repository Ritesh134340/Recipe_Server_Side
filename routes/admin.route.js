...
admin.put(
  ...
  async (req, res) => {
    try {
      ...
      const check = await Home.findOne({
        _id: req.body.dataId,
        carouselData: { $elemMatch: { image: req.body.image } },
      });
...
admin.delete(
  "/delete/data/:objectId/image/:imageId",
  authenticate,
  authorize("admin"),
  async (req, res) => {
    try {
      await Home.findOneAndUpdate({
        ...
        $pull: { carouselData: { _id: req.params.imageId } },
      });
...
admin.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.text) {
        updateFields["heroText"] = req.body.text;
      }
      if (req.body.image) {
        updateFields["heroImage"] = req.body.image;
      }

      await Home.findOneAndUpdate({ _id: req.body.objectId }, updateFields);
  ...
  async (req, res) => {
    try {
      ...
      if (req.query.publishedAt) {
        ...
        videoDocument = await Video.find()
