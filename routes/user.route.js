...
user.post("/add/favourite", authentication, async (req, res) => {
  await User.updateOne(
    ...
    { $push: { favourite: req.body.videoId } }
  );
    ...
    try {
      bcrypt.hash(password, 4, async function (err, hash) {
        if (hash) {
          ...
          const token = jwt.sign(
            { email: email, id: userDocument._id },
            process.env.SECRET_KEY
          );
    ...
    if (check) {
      ...
      if (mailerResponse.messageId) {
        try {
          ...
          res
            .status(200)
            .send({ mesg: "Otp sent successfully !", token: token });
        } catch (err) {
          res.status(500).send({ mesg: "Internal server error !" });
        }
      }
  ...
  try {
    ...
    if (check.otp === req.body.otp) {
      ...
      res
        .status(200)
        .send({
          mesg: "OTP verification successful !",
    ...
    if (decoded.verified) {
      bcrypt.hash(req.body.password, 4, async function (err, hash) {
        if (hash) {
          ...
          res
            .status(200)
            .send({ mesg: "Password updated successfully, please login!" });
        }
        if (err) {
...
user.patch("/edit/details", authentication, async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { email: req.params.email, id: updatedUser._id },
      process.env.SECRET_KEY
    );
    ...
    res
      .status(200)
      .send({
        mesg: "Profile updated successfully",
        token: token,
        profile: userData,
      });
  } catch (err) {
